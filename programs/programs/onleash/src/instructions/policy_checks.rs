...
pub fn enforce_policy_and_update_spend(
    ...
    if policy.allowlist_mode && !policy.allowlist.is_empty() && !policy.allowlist.contains(recipient) {
        return err!(OnLeashError::AddressNotAllowlisted);
    }

    let projected_daily = policy.spent_today
        .checked_add(amount)
    ...
    let vendor_spent = policy.vendor_entries
        .iter()
    ...
    if let Some(entry) = policy.vendor_entries.iter_mut().find(|e| e.address == *recipient) {
    ...
    if let Some(parent) = parent_policy {
        ...
        if parent.allowlist_mode && !parent.allowlist.is_empty() && !parent.allowlist.contains(recipient) {
            return err!(OnLeashError::ParentAddressNotAllowlisted);
        }

        let parent_daily_proj = parent.spent_today
            .checked_add(amount)
            .ok_or(OnLeashError::Overflow)?;
        ...
        let parent_vendor_spent = parent.vendor_entries
            .iter()
            .find(|e| e.address == *recipient)
            .map(|e| e.amount)
            .unwrap_or(0);
        ...
        if let Some(entry) = parent.vendor_entries.iter_mut().find(|e| e.address == *recipient) {
